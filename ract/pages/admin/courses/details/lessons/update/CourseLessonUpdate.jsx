        ...
        const fetchLesson = async () => {
            try {
                ...
                if (error) {
                    ...
                    if (data.video_file) {
                        setVideoPreview(data.video_file);
                    }

                    setValue(
                        "answer_form_pdf_url",
                        data.answer_form_pdf_url || ""
                    );
                    if (data.answer_form_pdf) {
                        setUploadedFilePDF(data.answer_form_pdf);
                    }
    ...
    const onSubmit = async (data) => {
        try {
            ...
            const { data: responseData, error } =
                await publicUpdateLessonInCourse(lessonId, formData);

            // console.log(`error`, error);
            // console.log(`responseData`, responseData);

            if (error) {
                Toast("error", error.message || "حدث خطأ أثناء تحديث الدرس");
            } else {
