...
import { publicCreateQuestionBank } from "../../../../api/public/authPublic";
...
function QuestionsBanksCreate() {
    ...
    const handleQuestionBankSubmit = async (formData) => {
        try {
            setIsLoading(true);

            const questionBankData = new FormData();
            questionBankData.append("user", userData?.id || null);
            questionBankData.append("section", formData.section);
            questionBankData.append("title", formData.title);
            questionBankData.append("description", formData.description || "");

            if (imageFile) questionBankData.append("image", imageFile);
            questionBankData.append("image_url", formData.image_url || "");
            questionBankData.append("is_visible", isVisible);

            const { data, error } = await publicCreateQuestionBank(
                questionBankData
            );
            ...
            Toast("error", "حدث خطأ أثناء إنشاء بنك الأسئلة");
        } finally {
            setIsLoading(false);
        }
    };
            ...
            <div className="">
                <div className="flex justify-start items-center gap-2 mb-8">
                    ...
                    <h1 className="text-3xl font-bold text-gray-800">
                        إضافة بنك اختبار جديد
                    </h1>
                </div>

                <div className="bg-white p-8 rounded-xl shadow-md">
                    <form
                        onSubmit={handleSubmit(handleQuestionBankSubmit)}
                    ...
                    >
                        ...
                        <div>
                            ...
                            <input
                                ...
                                className={`w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all`}
                                {...register("title", {
                                    required: "اسم بنك الاختبار مطلوب",
                                })}
                                autoComplete="off"
                                disabled={isSubmitting || isLoading}
                                required
                        ...
                        <div>
                            ...
                            <textarea
                                ...
                                name="description"
                                rows={4}
                                placeholder="مثال: بنك اختبار لمادة الرياضيات الصف الثالث الابتدائي"
...
export default QuestionsBanksCreate;
