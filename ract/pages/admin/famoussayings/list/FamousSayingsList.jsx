...
// api
import {
    publicGetFamousSayings,
    publicSearchFamousSayings,
    publicUpdateFamousSayingVisibility,
...
function FamousSayingsList() {
    ...
    const fetchCourses = async () => {
        try {
            ...
            const { data, error } = await publicGetFamousSayings(currentPage);

            // console.log(`error`, error);
            // console.log(`data`, data);

            if (error) {
                Toast("error", error.message || "حدث خطأ أثناء جلب الاقوال");
    ...
    const handleSearch = async () => {
        try {
            const { data, error } = await publicSearchFamousSayings(
                searchQuery,
                currentPage
            );
    ...
    const handleVisibilityChange = async (courseId, newVisibility) => {
        try {
            const { data, error } = await publicUpdateFamousSayingVisibility(
                courseId,
                newVisibility
            );
            ...
            if (error) {
                ...
                Toast("success", "تم تحديث حالة الاقوال بنجاح");
            }
        } catch (error) {
    ...
    const handleDelete = (courseId) => {
        setDeleteConfirmation({
            show: true,
            courseId,
            message: "هل أنت متأكد من حذف هذه الاقوال؟",
        });
    };

    const confirmDelete = async () => {
        try {
            const { error } = await publicDeleteFamousSaying(
                                            ...
                                            <div className="flex justify-between items-start">
                                                ...
                                                <div className="flex space-x-2">
                                                    ...
                                                    {userData?.id ===
                                                        course?.user?.id && (
                                                        <Link
                                                            to={`/${App_Admin}/famoussayings/update/${course?.id}`}
                                                            className={`p-2 text-gray-600 hover:text-blue-600 hover:bg-blue-50 rounded-md`}
                                                            title="تعديل الاقوال"
                                                        >
                                                            <Edit size={18} />
...
export default FamousSayingsList;
