...
function BooksList() {
    ...
    const fetchCourses = async () => {
        try {
            ...
            const { data, error } = await publicGetBooks(currentPage);

            // console.log(`error`, error);
            // console.log(`data`, data);

            if (error) {
                Toast("error", error.message || "حدث خطأ أثناء جلب الكتب");
            } else {
    ...
    const handleSearch = async () => {
        try {
            const { data, error } = await publicSearchBooks(
                searchQuery,
                currentPage
            );
    ...
    const handleVisibilityChange = async (courseId, newVisibility) => {
        try {
            const { data, error } = await publicUpdateBookVisibility(
                courseId,
                newVisibility
            );
    ...
    const handleDelete = (courseId) => {
        setDeleteConfirmation({
            show: true,
            courseId,
            message: "هل أنت متأكد من حذف هذه الكتاب؟",
        });
    };

    const confirmDelete = async () => {
        try {
            const { error } = await publicDeleteBook(
                                            ...
                                            <div className="flex justify-between items-start">
                                                ...
                                                <div className="flex space-x-2">
                                                    ...
                                                    {/* {userData?.id ===
                                                        course?.user?.id && ( */}
                                                    <Link
                                                        to={`/${App_Admin}/books/update/${course?.id}`}
                                                        className={`p-2 text-gray-600 hover:text-blue-600 hover:bg-blue-50 rounded-md`}
                                                        title="تعديل الكتاب"
                                                    >
                                                        <Edit size={18} />
