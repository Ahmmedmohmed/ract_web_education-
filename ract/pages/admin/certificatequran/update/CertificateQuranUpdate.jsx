...
// api
import {
    publicDeleteCertificateQuran,
    publicGetCertificateQuranById,
    publicUpdateCertificateQuran,
...
function CertificateQuranUpdate() {
    const { quranpathId, chapterinquranId, studentId, certificatequranId } =
        useParams();
    ...
    useEffect(() => {
        const fetchClassRoom = async () => {
            try {
                const { data, error } = await publicGetCertificateQuranById(
                    certificatequranId
                );
        ...
        fetchClassRoom();
    }, [certificatequranId, chapterinquranId, navigate, quranpathId, setValue]);
    ...
    const handleUpdate = async (formData) => {
        try {
            ...
            if (uploadedFilePDF instanceof File) {
                updateData.append("file_pdf", uploadedFilePDF);
            }
            updateData.append("file_url", formData.file_url || "");

            updateData.append("is_visible", isVisible);

            const { data, error } = await publicUpdateCertificateQuran(
                certificatequranId,
                updateData
            );
            ...
            if (error) {
                ...
                Toast("success", "تم تحديث السنه الدراسية بنجاح");
                navigate(
                    `/${App_Admin}/quranpaths/${quranpathId}/chapterinqurans/${chapterinquranId}`
                );
            }
    ...
    const handleDelete = async () => {
        try {
            ...
            const { error } = await publicDeleteCertificateQuran(
                certificatequranId
            );
            ...
            if (error) {
                ...
                Toast("success", "تم حذف السنه الدراسية بنجاح");
                navigate(
                    `/${App_Admin}/quranpaths/${quranpathId}/chapterinqurans/${chapterinquranId}`
                );
            }
                ...
                {showDeleteConfirm && (
                    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                        <div className="bg-white p-6 rounded-lg max-w-md w-full">
                            ...
                            <p className="mb-6">
                                هل أنت متأكد من حذف هذا السنه الدراسية؟ لا يمكن
...
export default CertificateQuranUpdate;
