...
// api
import {
    publicDeleteClassRoom,
    publicGetQuranPathsAdmin,
    publicGetClassRoomById,
    publicUpdateClassRoom,
...
function ClassRoomUpdate() {
    const { quranpathId, classroomId } = useParams();
    ...
    useEffect(() => {
        const fetchClassRoom = async () => {
            try {
                const { data, error } = await publicGetClassRoomById(
                    classroomId
                );
                ...
                if (error) {
                    ...
                    setValue(
                        ...
                        data.preservation_decision
                    );
                    setValue("associated_sciences", data.associated_sciences);
                    setValue("condition_acceptance", data.condition_acceptance);
                }
            } catch (error) {
        ...
        fetchClassRoom();
    }, [classroomId, navigate, setValue, quranPaths]);
    ...
    const handleUpdate = async (formData) => {
        try {
            ...
            const { data, error } = await publicUpdateClassRoom(
                classroomId,
                updateData
            );
    ...
    const handleDelete = async () => {
        try {
            ...
            const { error } = await publicDeleteClassRoom(classroomId);
