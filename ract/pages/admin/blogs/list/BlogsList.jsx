...
function BlogsList() {
    ...
    const fetchCourses = async () => {
        try {
            ...
            const { data, error } = await publicGetBlogs(currentPage);

            // console.log(`error`, error);
            // console.log(`data`, data);

            if (error) {
                Toast("error", error.message || "حدث خطأ أثناء جلب المقالات");
            } else {
    ...
    const handleSearch = async () => {
        try {
            const { data, error } = await publicSearchBlogs(
                searchQuery,
                currentPage
            );
    ...
    const handleVisibilityChange = async (courseId, newVisibility) => {
        try {
            const { data, error } = await publicUpdateBlogVisibility(
                courseId,
                newVisibility
            );
            ...
            if (error) {
                ...
                Toast("success", "تم تحديث حالة المقالة بنجاح");
            }
        } catch (error) {
    ...
    const confirmDelete = async () => {
        try {
            const { error } = await publicDeleteBlog(
                                    ...
                                    <div className="flex flex-col md:flex-row p-4">
                                        ...
                                        <div className="flex-grow flex flex-col justify-between ">
                                            <div className="flex justify-between items-start">
                                                ...
                                                >
                                                    <h2
                                                        className={`text-3xl font-bold text-black hover:text-blue-600 transition-all duration-500 hover:transition-all hover:duration-500 group-hover:text-blue-600  line-clamp-1`}
                                                    >
                                                        {course?.title}
                                                    </h2>
                                                </Link>
                                                ...
                                                <div className="flex space-x-2">
                                                    ...
                                                    {userData?.id ===
                                                        course?.user?.id && (
                                                        <Link
                                                            to={`/${App_Admin}/blogs/update/${course?.id}`}
                                                            className={`p-2 text-gray-600 hover:text-blue-600 hover:bg-blue-50 rounded-md`}
                                                            title="تعديل المقالة"
                                                        >
                                                            <Edit size={18} />
                                            ...
                                            <p className="text-2xl text-gray-600 -mt-2 mb-4 pe-20 line-clamp-1">
                                                {course?.description}
                                            </p>

                                            <div className="flex flex-wrap gap-4 text-lg  text-gray-500 mt-auto mb-2">
