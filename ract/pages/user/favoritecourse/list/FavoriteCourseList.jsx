...
import { userGetStudentFavoriteCourses } from "../../../../api/user/authUser";

function FavoriteCourseList() {
    ...
    const fetchCourses = async () => {
        try {
            ...
            const { data, error } = await userGetStudentFavoriteCourses(
                userData?.id,
                currentPage
            );
            ...
            if (error) {
                Toast(
                    "error",
                    error.message || "حدث خطأ أثناء جلب الدورات المفضلة"
                );
            } else {
        ...
        <>
            <div className="space-y-6">
                ...
                {isLoading ? (
                    ...
                    <div className="text-2xl text-center py-12 bg-white rounded-lg">
                        <p className="text-gray-500">
                            لم تقم باضافة دورة الي المفضلات بعد
                        </p>
                    </div>
                ) : (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {courses?.map((course, index) => (
                            <FavoriteCourseCard key={index} enroll={course} />
                        ))}
                    </div>
                )}
