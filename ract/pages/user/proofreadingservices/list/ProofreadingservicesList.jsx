...
// API
import {
    publicGetProofreadingServices,
    publicSearchProofreadingServices,
    publicUpdateProofreadingServiceStatus,
    publicDeleteProofreadingService,
...
function ProofreadingservicesList() {
    ...
    const fetchMessages = async () => {
        try {
            ...
            const { data, error } = await publicGetProofreadingServices(
                currentPage,
                selectedStatus
            );
            ...
            if (error) {
                Toast("error", error.message || "حدث خطأ أثناء جلب التدقيق");
            } else {
    ...
    const handleSearch = async () => {
        try {
            const { data, error } = await publicSearchProofreadingServices(
                searchQuery,
                currentPage
            );
    ...
    const handleStatusChange = async (messageId, newStatus) => {
        try {
            const { data, error } = await publicUpdateProofreadingServiceStatus(
                messageId,
                newStatus
            );
            ...
            if (error) {
                ...
                Toast("success", "تم تحديث حالة التدقيق بنجاح");
            }
        } catch (error) {
    ...
    const confirmDelete = async () => {
        try {
            const { error } = await publicDeleteProofreadingService(
    ...
    const handleDelete = (messageId) => {
        setDeleteConfirmation({
            show: true,
            messageId,
            message: "هل أنت متأكد من حذف هذه التدقيق؟",
        });
    };
                    ...
                    {isLoading ? (
                        ...
                        <div className="text-center py-12 bg-white rounded-lg">
                            <p className="text-gray-500">
                                لا لم يتم إرسال تدقيق لغوي حتي الان
                            </p>
                        </div>
                    ) : (
                                ...
                                <div className="p-5">
                                    <div className="flex justify-between items-start">
                                        <div>
                                            ...
                                            <div className="my-4 space-y-1 flex flex-col md:flex-row gap-4 ">
                                                <div className="flex items-center text-lg text-black">
                                                    <a
                                                        className={`flex items-center gap-2 text-lg text-black transition-all duration-500 hover:text-blue-600`}
                                            ...
                                            <div className="my-4 space-y-1 flex flex-col flex-wrap md:flex-row gap-4  ">
                                                ...
                                                <div className="flex items-center gap-2 text-lg text-black">
                                                    ...
                                                    <span>مدة الاستلام </span>

                                                    <span>
                                                        (
                                                        {
                                                            message?.receipt_period
                                                        }
                                                        )
                                                    </span>
                                                </div>
                                            ...
                                            {activeDropdown === message.id && (
                                                <div className="absolute left-0 bg-white shadow-lg rounded-md py-1 w-40 z-10 transition-all">
                                                    <button
                                                        onClick={() => {
                                                            navigate(
                                                                `/${App_User}/proofreadingservices/update/${message.id}`
                                                            );
                                                        }}
                                                        className={`flex items-center px-4 py-2 text-lg text-blue-600 hover:bg-gray-100 w-full text-right`}
                                                    >
                                                        <Edit
                                                            size={16}
                                                            className="ml-2"
                                                    ...
                                                    <button
                                                        ...
                                                        className="flex items-center px-4 py-2 text-lg text-red-600 hover:bg-gray-100 w-full text-right"
                                                    >
                                                        <Trash2
                                                            size={16}
                                                            className="ml-2"
                                    ...
                                    <div className="flex items-center justify-between mt-4 pt-2 border-t border-gray-100">
                                        <div>
                                            {message?.quick_reply && (
                                                <button
                                                    className={`text-lg text-blue-600 hover:text-blue-700 flex items-center`}
                                                >
                                                    <ClockFading
                                                        size={14}
                                                        className="ml-1"
...
export default ProofreadingservicesList;
