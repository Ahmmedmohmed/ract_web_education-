...
// api
import {
    publicGetPowerpoints,
    publicSearchPowerpoints,
    publicUpdatePowerpointVisibility,
    publicDeletePowerpoint,
} from "../../../../api/public/authPublic";
import { userGetStudentEnrolledPowerpoints } from "../../../../api/user/authUser";
...
function PowerpointsList() {
    ...
    const fetchCourses = async () => {
        try {
            ...
            const { data, error } = await userGetStudentEnrolledPowerpoints(
                userData?.id
            );
    ...
    const handleSearch = async () => {
        try {
            const { data, error } = await publicSearchPowerpoints(
                searchQuery,
                currentPage
            );
    ...
    const handleVisibilityChange = async (courseId, newVisibility) => {
        try {
            const { data, error } = await publicUpdatePowerpointVisibility(
                courseId,
                newVisibility
            );
    ...
    const confirmDelete = async () => {
        try {
            const { error } = await publicDeletePowerpoint(
    ...
    return (
        <>
            <div className="space-y-6">
                ...
                {/* Search and filter */}
                <div className="bg-white rounded-lg shadow-sm p-6 mb-10 hidden">
                    ...
                    {isLoading ? (
                        ...
                        <div className="text-center py-12 bg-white rounded-lg">
                            <p className="text-gray-500">
                                لم يتم اضافة بوربوينت حتي الان
                            </p>
                        </div>
                    ) : (
                                    ...
                                    <div className="flex flex-col md:flex-row p-4">
                                        <div
                                            ...
                                            onClick={() => {
                                                ...
                                                // ) {
                                                //     navigate(
                                                //         `/${App_Admin}/powerpoints/${course?.id}`
                                                //     );
                                                // }
                                            }}
                                        >
                                            <img
                                                src={
                                                    course?.powerpoint
                                                        ?.image_url ||
                                                    course?.powerpoint?.image ||
                                                    noimage
                                                }
                                                alt={course?.powerpoint?.title}
                                                onError={(e) => {
                                        ...
                                        <div className="flex-grow flex flex-col justify-between ">
                                            <div className="flex justify-between items-start">
                                                ...
                                                {/* <div className="flex space-x-2">
                                                    ...
                                                    {userData?.id ===
                                                        course?.user?.id && (
                                                        <Link
                                                            to={`/${App_Admin}/powerpoints/update/${course?.id}`}
                                                            className={`p-2 text-gray-600 hover:text-blue-600 hover:bg-blue-50 rounded-md`}
                                                            title="تعديل البوربوينت"
                                                        >
                                                            <Edit size={18} />
                                                        </Link>
                                                    )}
                                            ...
                                            {/* <div className="flex flex-wrap gap-4 text-lg text-gray-500 mt-auto mb-4">
