...
// API
import {
    publicGetPowerpointsServices,
    publicSearchPowerpointsServices,
    publicUpdatePowerpointServiceStatus,
    publicDeletePowerpointService,
...
function PowerpointservicesList() {
    ...
    const fetchMessages = async () => {
        try {
            ...
            const { data, error } = await publicGetPowerpointsServices(
                currentPage,
                selectedStatus
            );
            ...
            if (error) {
                Toast("error", error.message || "حدث خطأ أثناء جلب البوربوينت");
            } else {
    ...
    const handleSearch = async () => {
        try {
            const { data, error } = await publicSearchPowerpointsServices(
                searchQuery,
                currentPage
            );
    ...
    const handleStatusChange = async (messageId, newStatus) => {
        try {
            const { data, error } = await publicUpdatePowerpointServiceStatus(
                messageId,
                newStatus
            );
            ...
            if (error) {
                ...
                Toast("success", "تم تحديث حالة البوربوينت بنجاح");
            }
        } catch (error) {
    ...
    const confirmDelete = async () => {
        try {
            const { error } = await publicDeletePowerpointService(
    ...
    const handleDelete = (messageId) => {
        setDeleteConfirmation({
            show: true,
            messageId,
            message: "هل أنت متأكد من حذف هذه البوربوينت",
        });
    };
                    ...
                    {isLoading ? (
                        ...
                        <div className="text-center py-12 bg-white rounded-lg">
                            <p className="text-gray-500">
                                لا لم يتم إرسال طلبات بوربوينت حتي الان
                            </p>
                        </div>
                    ) : (
                                            ...
                                            {activeDropdown === message.id && (
                                                <div className="absolute left-0 bg-white shadow-lg rounded-md py-1 w-40 z-10 transition-all">
                                                    <button
                                                        onClick={() => {
                                                            navigate(
                                                                `/${App_User}/powerpointservices/update/${message.id}`
                                                            );
                                                        }}
...
export default PowerpointservicesList;
