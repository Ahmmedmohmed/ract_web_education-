...
// api
import {
    publicUpdatePowerpointService,
    publicGetPowerpointServiceById,
...
function PowerpointserviceUpdate() {
    ...
    useEffect(() => {
        const fetchCourse = async () => {
            try {
                ...
                const { data, error } = await publicGetPowerpointServiceById(
                    proofreadingId
                );
                ...
                if (error) {
                    Toast(
                        "error",
                        error.message || "حدث خطأ أثناء جلب طلب البوربوينت"
                    );
                    navigate(`/${App_User}/powerpointservices`);
                } else {
                    // Set form values
                    setValue("mold_shape", data.mold_shape);
                ...
                Toast("error", "حدث خطأ أثناء جلب طلب البوربوينت");
                navigate(`/${App_User}/powerpointservices`);
            } finally {
                setIsLoading(false);
            }
    ...
    const handleCourseSubmit = async (formData) => {
        try {
            ...
            const { data, error } = await publicUpdatePowerpointService(
                proofreadingId,
                courseData
            );
...
export default PowerpointserviceUpdate;
